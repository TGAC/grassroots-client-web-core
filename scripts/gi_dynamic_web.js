                ...
                "columns": [
                    {
                        ...
                        "render": function (data, type, full, meta) {
                            ...
                            return '<a class="newstyle_link" href="/dynamic-web-test/service/' + full['so:name'] + '"><img src="' + full['operation']['so:image'] + '"/> <u>' + full['so:name'] + '</u></a>';
                        }
                    },
...
function do_ajax_search() {
    ...
    if (input.length > 1) {
        ...
        $('#ajax_result').html('Searching <img src=\"/dynamic-web-test/images/ajax-loader.gif\"/>');
        var timer;
...
function submit_form() {
    $('#status').html('<img src="/dynamic-web-test/images/ajax-loader.gif"/>');
...
function display_result(json) {
    ...
    if (selected_service_name == 'BlastN' || selected_service_name == 'BlastP' || selected_service_name == 'BlastX') {
        ...
        for (var i = 0; i < json['results'].length; i++) {
            ...
            $('#result').append('<fieldset><legend>' + dbname + '</legend><div><p><b>Job ID: ' + uuid + '</b></p><div id=\"' + uuid + '\">Job Submitted <img src=\"/dynamic-web-test/images/ajax-loader.gif\"/></div></div></br></fieldset>');
        ...
        $.ajax({
                ...
                success: function (json) {
                    ...
                    if (status_text_key == 'Partially succeeded' || status_text_key == 'Succeeded') {
                        ...
                        jQuery('#' + uuid).html('Job ' + status_text_key + ' <img src=\"/dynamic-web-test/images/ajax-loader.gif\"/>');
                        var timer;
                        clearTimeout(timer);
                        timer = setTimeout(function () {
                            checkResult(each_result);
...
function downloadJobFromServer(id) {
    $('#' + id + 'status').html('<img src="/dynamic-web-test/images/ajax-loader.gif"/>');
